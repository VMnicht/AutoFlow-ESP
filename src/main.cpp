...
// 收到任务(ID=1)或扫码开始后，语音播报完毕等待多久开始执行任务 (单位: 毫秒)
const unsigned long DELAY_BEFORE_TASK_START = 15000; 
const unsigned long DELAY_BEFORE_TASK_START2 = 5000; 

// 收到复位(ID=2)后，语音播报完毕等待多久恢复默认全开 (单位: 毫秒)
const unsigned long DELAY_BEFORE_RESET = 1000;

// 【新增】液位传感器使能开关
// true = 启用该通道传感器检测, false = 忽略该通道传感器
// 对应通道: {通道1, 通道2, 通道3, 通道4}
const bool ENABLE_SENSORS[4] = {false, true, true, false}; 
...
// 【新增】触发扫码控制引脚脉冲 (Low -> High -> Delay -> Low)
void triggerScanPulse() {
  digitalWrite(PIN_SCAN_CTRL, HIGH);
  delay(200); // 维持 200ms 高电平脉冲
...
void processInfusionLogic() {
  ...
  if (taskMgr.currentStepIndex >= 4) {
    ...
    // 【新增需求】：任务结束时，加入一次引脚跳变 (High -> Low -> High -> Low)
    // 1. 任务结束，isRunning 变 false，主循环稍后会拉低引脚，但我们这里为了跳变效果明显，手动控制
    digitalWrite(PIN_SCAN_CTRL, LOW); 
    delay(100); // 短暂停顿，确保从运行态的高电平先拉低，产生上升沿
...
void checkSensors() {
  ...
  for (int i = 0; i < 4; i++) {
    // 逻辑：(该通道有任务时长) AND (该通道被配置为启用检测)
    if (taskMgr.durations[i] > 0 && ENABLE_SENSORS[i]) {
  ...
  if (Serial2.available()) {
    ...
    // 指令 2: 开始任务
      ...
      if (idIdx > 0) {
        ...
        // 【修改】扫码开始任务后，不再立即执行，而是进入等待逻辑（同MQTT）
        taskMgr.isDefaultOpen = false; // 先关掉默认全开
        stopTaskLogic();               // 清除可能存在的旧状态
        
        // 设置待执行动作
        pendingAction.type = ACTION_START_TASK;
        pendingAction.triggerTime = millis() + DELAY_BEFORE_TASK_START2;
...
void loop() {
  ...
  // 需求：
  // 1. 任务进行时始终为高电平
  // 2. 暂停所有任务（不执行）或 默认模式下 为低电平
  // 3. 扫码消息和任务结束时会有脉冲（由 handleSerialInput 和 processInfusionLogic 触发）
  
  if (taskMgr.isRunning && !taskMgr.isPaused) {
    digitalWrite(PIN_SCAN_CTRL, HIGH);
  } else {
    // 只有在非运行状态下才允许拉低
    // 注意：如果是 triggerScanPulse() 正在执行中，那是阻塞延时，不会走到这里被覆盖
    digitalWrite(PIN_SCAN_CTRL, LOW);
  }
